...
epicalc_profile <- function(data,lv1_low=60,lv2_low=55,lv1_high=85,lv2_high=100,
  ...
  episodes = episode_calculation(data, lv1_low = lv1_low, lv2_low = lv2_low,
                                 lv1_high = lv1_high, lv2_high = lv2_high,
  ...
  #Creating table 1(t1) -------------------------------------
  tableStat = data.frame("Low/High episode metrics")
  tableStat[1, 1] = ""
  tableStat[1, 2] = "low"
  tableStat[1, 3] = "low"
  tableStat[1, 4] = "low"
  ...
  tableStat[3, 2] = paste0("<", as.character(lv1_low), " BPM")
  tableStat[3, 3] = paste0("<", as.character(lv2_low), " BPM")
  tableStat[3, 4] = paste0("<", as.character(lv1_low), " BPM")
  tableStat[3, 5] = paste0(">", as.character(lv1_high), " BPM")
  tableStat[3, 6] = paste0(">", as.character(lv2_high), " BPM")
  tableStat[3, 7] = paste0(as.character(lv1_low), "-", as.character(lv2_low), " BPM")
  tableStat[3, 8] = paste0(as.character(lv1_high), "-", as.character(lv2_high), " BPM")
  ...
  tableStat[6, 1] = "Mean Heart rate"
  tableStat[6, 2] = paste0(as.character(format(round(ep_summary$avg_ep_hr[1], 2), nsmall = 2)), " BPM")
  tableStat[6, 3] = paste0(as.character(format(round(ep_summary$avg_ep_hr[2], 2), nsmall = 2)), " BPM")
  tableStat[6, 4] = paste0(as.character(format(round(ep_summary$avg_ep_hr[3], 2), nsmall = 2)), " BPM")
  tableStat[6, 5] = paste0(as.character(format(round(ep_summary$avg_ep_hr[4], 2), nsmall = 2)), " BPM")
  tableStat[6, 6] = paste0(as.character(format(round(ep_summary$avg_ep_hr[5], 2), nsmall = 2)), " BPM")
  tableStat[6, 7] = paste0(as.character(format(round(ep_summary$avg_ep_hr[6], 2), nsmall = 2)), " BPM")
  tableStat[6, 8] = paste0(as.character(format(round(ep_summary$avg_ep_hr[7], 2), nsmall = 2)), " BPM")
  ...
  labels = c("lv1_low", "lv2_low", "lv1_high", "lv2_high")
  plot_data = ep_data |>
    dplyr::rowwise() |>
    dplyr::mutate(
      num_levels = sum(c(lv1_low != 0, lv2_low != 0, lv1_high != 0, lv2_high != 0)),
      class = ifelse(
        # either no types - normal, one type - keep nonzero, subset - choose lv2
        num_levels == 0, "Normal",
        ifelse(num_levels == 1,
               labels[which(c(lv1_low != 0, lv2_low != 0, lv1_high != 0, lv2_high != 0))],
               c("lv2_low", "lv2_high")[which(c(lv2_low != 0, lv2_high != 0))])
      ),
      class = factor(class, levels = c("lv2_low", "lv1_low", "Normal", "lv1_high", "lv2_high"))
      )
