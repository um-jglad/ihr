...
The ihr package is designed to help in the analyses of heart rate data from wearable technologies (i.e. FitBit, Apple Watches, etc.). These wearable technologies capture the heart rate of the user while the user is wearing the device, often in intervals of only a couple of seconds. Wearable technologies commonly track heart rate using Photoplethysmography (PPG). Data from these wearable are increasingly used to understand a user's physical activity and their heart functionality on a day by day basis. 
...
percent_nonmissing represents the completeness of heart rate recordings as a percentage at the day level.
